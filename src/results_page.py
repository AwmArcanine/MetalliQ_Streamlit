...
def results_page(results: Optional[dict] = None, ai_text: Optional[Any] = None):
    ...
    with st.container():
        st.markdown("<div class='mq-neon'><div class='mq-title'>Export & Actions</div><div class='mq-sub'>Save or export report sections</div>", unsafe_allow_html=True)
        # flattened CSV exports for core tables
        if st.button("Download Full Report JSON"):
            st.download_button("Download JSON", data=json.dumps(results, indent=2), file_name="lca_report.json", mime="application/json")
        st.markdown("</div>", unsafe_allow_html=True)
